...
const { getNASAFact } = require("../utils/nasaApi");
...
async function connectToRegistry() {
  ...
  connecting.succeed(chalk.green("[OK] Connected to registry.do-nothing.io"));
}
...
async function verifyChecksums() {
  ...
  if (Math.random() > 0.6) {
    ...
    console.log(chalk.red("   [ERROR] Hash mismatch for noop@5.1.2"));
    await sleep(600);
...
async function buildNativeModules() {
  ...
  if (Math.random() > 0.7) {
    ...
    console.log(chalk.red("   [ERROR] Compiler error in useless-lib@0.0.1"));
    await sleep(500);
